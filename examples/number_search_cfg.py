#!/usr/bin/env python3
"""Number-search rollouts with a genlm BoolCFG constraint over ``<thought>`` + ``<python>guess(...)``.

The constraint matches Platoon's default NumberSearch prompt shape and keeps completions to the
expected tagged thought/code format.

**Sharding (20 tasks on 2 GPUs):** use half-open ranges ``[task_start, task_end)`` and the same
``--output-dir`` (or the default under ``--output-root``). Event files include a rollout UUID, so
shards do not collide. Within each process, rollouts run sequentially.

    CUDA_VISIBLE_DEVICES=0 python examples/number_search_cfg.py --task-start 0 --task-end 10
    CUDA_VISIBLE_DEVICES=1 python examples/number_search_cfg.py --task-start 10 --task-end 20

Or pass ``--cuda-device N`` as the first arguments (stripped before imports).
"""
...
# Strict layout (ENTIRE completion must match, no extra text/code):
#   <thought> ... </thought>
#   <python> guess(<int>) </python>
NUMBER_SEARCH_THOUGHT_PYTHON_CFG = r"""
start: THOUGHT_BLOCK SEP PY_BLOCK TRAILING_WS

// Keep tag contents clean by allowing optional whitespace, but do not force leading/trailing
// newlines inside the tags.
THOUGHT_BLOCK: "<thought>" THOUGHT_BODY "</thought>"
PY_BLOCK: "<python>" WS "guess" WS "(" WS INT WS ")" WS "</python>"

// Disallow '<' inside the thought body so we do not accidentally consume the closing tag.
THOUGHT_BODY: /[^<]*/
...
def _default_output_dir(args: argparse.Namespace) -> str:
    ...
    sub = f"number_search_cfg_{args.split}"
...
def main() -> None:
    p = argparse.ArgumentParser(description="Number-search + CFG structure constraint.")
    p.add_argument("--model", default=DEFAULT_MODEL, help="HF model id for PromptedLLM")
    p.add_argument("--split", choices=("train", "val"), default="val")
    p.add_argument("--task-start", type=int, default=0, help="First task index (inclusive)")
    p.add_argument("--task-end", type=int, default=20, help="One past last task index (exclusive)")
    ...
    p.add_argument(
        "--output-dir",
        default=None,
        help="Shared rollout directory for all shards (default: OUTPUT_ROOT/number_search_cfg_SPLIT/)",
    )
    p.add_argument("--verbose", action="store_true", help="Platoon rollout logging")
    args = p.parse_args()
